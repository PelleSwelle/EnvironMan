class Grabber
{
    ...
    void display()
    {
        ...
        {
            if (environman.facingRight) // if facing right
            {
                switchSprite(spriteIdle_R);
                xpos = environman.xpos + environman.currentSprite.width - xInset;
            }
            else if (!environman.facingRight)
            {
                switchSprite(spriteIdle_L);
                xpos = environman.xpos - currentSprite.width + xInset;
            }
        }
        else if (isExtended)
        {
        ...
        ypos = environman.ypos + environman.currentSprite.height / 2;
    ...
    {
        noFill();
        if (environman.facingRight)
        {
            rect(xpos, ypos, currentSprite.width, currentSprite.height);
        }
        else if (!environman.facingRight)
        {
            rect(environman.xpos - currentSprite.width, ypos, currentSprite.width, currentSprite.height);
        }
